...
# Cache the DiskAnn platform check so we only evaluate it once per session.
_DISKANN_PRELOAD_REASON: str | None = None
_DISKANN_PRELOAD_DONE: bool = False


def _ensure_diskann_runtime_or_reason() -> str | None:
    """Return None if DiskAnn is available on this platform, or a human-readable
    skip reason on unsupported platforms. Idempotent across calls."""
