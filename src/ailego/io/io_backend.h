...
//
// Wraps the low-level I/O backends (io_uring via raw syscalls, LibAioLoader
// for libaio) and provides a uniform way to initialize, query, and report the
...
enum class IOBackendType {
  ...
  kIoUring,    // io_uring via raw kernel syscalls (zero dependency)
...
class IOBackend {
 public:
  ...
  // Try to load the best available backend (io_uring > libaio > sync_pread).
