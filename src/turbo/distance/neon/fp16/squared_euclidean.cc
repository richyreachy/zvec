...
void squared_euclidean_fp16_distance(const void *a, const void *b, size_t dim,
  ...
  for (; lhs != last_aligned; lhs += 8, rhs += 8) {
    ...
    const float32x4_t diff0 = vsubq_f32(vcvt_f32_f16(vget_low_f16(lhs8)),
                                        vcvt_f32_f16(vget_low_f16(rhs8)));
    const float32x4_t diff1 =
        vsubq_f32(vcvt_high_f32_f16(lhs8), vcvt_high_f32_f16(rhs8));
    sum0 = vfmaq_f32(sum0, diff0, diff0);
    sum1 = vfmaq_f32(sum1, diff1, diff1);
  }
  if (last - lhs >= 4) {
    const float32x4_t diff =
        vsubq_f32(vcvt_f32_f16(vld1_f16(lhs)), vcvt_f32_f16(vld1_f16(rhs)));
  ...
  for (; lhs != last; ++lhs, ++rhs) {
    const float diff = static_cast<float>(*lhs) - static_cast<float>(*rhs);
    result += diff * diff;
  }
  *distance = result;
#else
