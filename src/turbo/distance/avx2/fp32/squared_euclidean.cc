...
void squared_euclidean_contiguous(const float *block, const float *query,
  ...
  for (size_t i = 0; i < n; ++i, vec += dim) {
    ...
    for (; d + 16 <= dim; d += 16) {
      const __m256 diff0 =
          _mm256_sub_ps(_mm256_loadu_ps(query + d), _mm256_loadu_ps(vec + d));
      acc0 = _mm256_fmadd_ps(diff0, diff0, acc0);
      const __m256 diff1 = _mm256_sub_ps(_mm256_loadu_ps(query + d + 8),
                                         _mm256_loadu_ps(vec + d + 8));
...
void squared_euclidean_fp32_contiguous_batch_distance_avx2(const void *block,
                                                           const void *query,
                                                           size_t n, size_t dim,
                                                           float *distances) {
#if defined(__AVX2__)
