...
void squared_euclidean_fp16_contiguous_batch_distance(const void *block,
                                                      const void *query,
                                                      size_t n, size_t dim,
                                                      float *distances) {
  const ailego::Float16 *vec = reinterpret_cast<const ailego::Float16 *>(block);
  for (size_t i = 0; i < n; ++i, vec += dim) {
    squared_euclidean_fp16_distance(vec, query, dim, &distances[i]);
  }
}
